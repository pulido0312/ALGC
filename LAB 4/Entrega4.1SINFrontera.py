...
def dijkstra(E,inic,fin):
    ...
    while noVisitados:
        nodoMenorDistancia = None
        for nodo in noVisitados:
            if nodoMenorDistancia is None:
                nodoMenorDistancia = nodo
            elif menorDistancia[nodo] < menorDistancia[nodoMenorDistancia]:
                nodoMenorDistancia = nodo
        if(nodoMenorDistancia==fin):
            break        
        opcionesCamino = E[nodoMenorDistancia].items()
        for nodoHijo,ejeran in opcionesCamino:
            if ejeran + menorDistancia[nodoMenorDistancia]<menorDistancia[nodoHijo]:
                menorDistancia[nodoHijo]= ejeran + menorDistancia[nodoMenorDistancia]
                predecesor[nodoHijo]=nodoMenorDistancia
        noVisitados.pop(nodoMenorDistancia)
    nodoActual = fin
